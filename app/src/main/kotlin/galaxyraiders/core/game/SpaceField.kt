...
data class SpaceField(val width: Int, val height: Int, val generator: RandomGenerator) {
  ...
  private fun defineMissilePosition(missileRadius: Double): Point2D {
    return ship.center + Vector2D(
      dx = 0.0, dy = ship.radius + missileRadius + SpaceFieldConfig.missileDistanceFromShip)
  }
  ...
  fun generateExplosion(pointOfExplosion: Point2D) {
    explosions += createExplosion(pointOfExplosion)
  }

  private fun createExplosion(pointOfExplosion: Point2D): Explosion {
    return Explosion(pointOfExplosion)
  }
